...
def makeEventDisplays(selection,run,version,tag,maxEvents):
    offlineDir = "/store/data_offline/Run{0}/MilliQan_Run{0}.*_formosa.root".format(run,version)
    chain = r.TChain("t")
    chain.Add(offlineDir)
    table = findEvents.main(chain,run,maxEvents,tag,selection)
    for run,fileNumber,eventNumber in table:
        inputFile = "/backup/archive/Run{0}/MilliQan_Run{0}.{1}_formosa.root".format(run,fileNumber)
        runOfflineFactory(inputFile,"temp.root",exe,None,False,force_publish=False,database=None,appendToTag=tag,mergedTriggerFile="",drs=False,display=[eventNumber],slab=False,formosa=True,runNumber=run,fileNumber=fileNumber)
