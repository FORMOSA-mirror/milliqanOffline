#For comparing Matthew's method of calculating rates with one including a trigger-able time cut for through-going non-panel activity in the detctor.
import uproot
import awkward
import sys
import os
...
outfile = ROOT.TFile("outputs/exploringWindowTiming_comparingRates_timeCut_run740_withTriggers.root", "RECREATE")
...
for data in uproot.iterate(filelist, variables, library='ak', step_size=100000): 
    ...
    #Broadcast event-level variables to proper shape
...
times.SetTitle("event_time_fromTDC for Through-Going Non-Panel Activity")
times.GetXaxis().SetTitle("Event Time [ns]")
times.Write()

times_withCut.SetTitle("event_time_fromTDC for Through-Going Non-Panel Activity, Triggerable Time Cut")
...
rates.GetXaxis().SetTimeFormat("#splitline{%y-%m-%d}{%H:%M:%S}%F1970-01-01 01:00:00")
...
rates_withCut.GetXaxis().SetTimeFormat("#splitline{%y-%m-%d}{%H:%M:%S}%F1970-01-01 01:00:00")
...
rates.SetTitle("Rates for Through-Going Non-Panel Activity")
rates.GetXaxis().SetTitle("Event Time [ns]")
rates.GetYaxis().SetTitle("Rate [Hz]")
rates.Write()

rates_withCut.SetTitle("Rates for Through-Going Non-Panel Activity, Triggerable Time Cut")
