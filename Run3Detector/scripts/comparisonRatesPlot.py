#For generating a comparison rates plot for through-going stuff in the FORMOSA detector
import ROOT
...
hist7.SetFillColor(5)
hist7.SetLineColor(5)
...
hist1.Draw("same")
stack.Draw("HISTsame")
stack.SetTitle("Comparing Through-Going Populations, Run709, Loose Timing Cut")
hist1.Draw("same")

# Create legend
legend = ROOT.TLegend(0.5, 0.6, 0.9, 0.9)  # Adjust coordinates as needed
legend.AddEntry(hist1, "All through-going activity", "l")
legend.AddEntry(hist2, "Through-going activity, no panels", "f")
legend.AddEntry(hist3, "Through-going activity, with panels, below panel threshold", "f")
legend.AddEntry(hist4, "Through-going activity, with panels, above panel threshold", "f")
legend.AddEntry(hist6, "Through-going, with one panels", "f")
legend.AddEntry(hist7, "Through-going, with two panels (only one above threshold)", "f")
legend.AddEntry(hist5, "Leftover Through-Going Events", "f")
legend.Draw()
